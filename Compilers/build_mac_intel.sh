...
dotnet publish inFAMOUSReborn.csproj -c Release -r osx-x64 --self-contained true -p:UseAppHost=true -p:Version=$VERSION -o ./temp_backend
dotnet publish inFAMOUSReborn.Launcher/inFAMOUSReborn.Launcher.csproj -c Release -r osx-x64 --self-contained true -p:UseAppHost=true -p:Version=$VERSION -o ./temp_launcher
...
echo "MacOS Intel build compiled. Happy trophy hunting! :)"